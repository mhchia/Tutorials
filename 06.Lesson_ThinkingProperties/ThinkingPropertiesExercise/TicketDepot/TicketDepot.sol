...
contract TicketDepot {
	...
	function getEventData(uint16 _eventID) view public returns (address, uint64, uint16) {
		Event memory e = eventsMap[_eventID];
		return (e.owner, e.ticketPrice, e.ticketsRemaining);
	}

	function getAttendee(uint16 _eventID, uint16 _ticketID) view public returns (address) {
		return eventsMap[_eventID].attendees[_ticketID];
	}

	function getNumEvents() view public returns (uint16) {
		return numEvents;
	}
